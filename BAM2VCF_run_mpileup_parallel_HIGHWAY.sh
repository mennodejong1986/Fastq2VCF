...
BCFTOOLS=/opt/software/bcftools-1.16/bcftools								# Path to bcftools executable, at least: git version 1.11-24-g9718479+, older versions do not contain bcftools call --group-samples option
...
REFERENCE=/home/mdejong/deerproject/genome/Cervus_canadensis_genome.fna		# Path to unzipped reference genome.
BAMFILES=Kashmirbamfiles.txt												# a txt file with (full path and) names of input bam files, one per line. Bam-files should already be indexed with samtools index command.
POPFILE=groupfile_Kashmir.txt												# necessary for callgenotypes step; tab separated txt-file with two columns (no header): sample name and population name. 
...
NRSETS=18
#NRSETS=25						# Number of subdivisions (this is basically the number of commands you want to run parallel). 
								...
								# You might want to remove the scaffold of the X-chrom and Y-chrom (in case you have identified them) and run the analyses on these scaffolds separately, with different ploidy settings
PREFIX=Kashmir					# Desired prefix of output files. Should NOT contain a period (.), otherwise the sorting of the subset files will be corrupted. 
...
# SITE FILTERING:
calcdepth2=TRUE			## like calcdepth option, but uses masked vcf files as input (instead of unmasked), generated by runpipeline. 
...
mindepth=750			## around 6x nrsamples (mindepth) 
maxdepth=2150			## around 1.5x overall meandepth (e.g: if mean depth per sample is 10, and you have 100 samples, then: 10*100*1.5 = 1500)
...
if [[ "$calcdepth" = TRUE ]]
	...
	${VCFTOOLS} --gzvcf ${PREFIX}.mybed1.txt.vcf.gz --depth --out sampledepth &
	#
	echo "Extracting sample names using bcftools query..."
	${BCFTOOLS} query --list-samples ${PREFIX}.mybed1.txt.vcf.gz > myinfo.samples.txt &
	echo "Thinning data using vcftools..."
	${VCFTOOLS} -c --thin 100 --gzvcf ${PREFIX}.mybed1.txt.vcf.gz --recode --recode-INFO-all | gzip > ${PREFIX}.mybed1.txt.thin100.vcf.gz
	echo "Extracting depth information from thinned dataset using bcftools query..."
	${BCFTOOLS} query -f '%DP[\t%DP]\n' ${PREFIX}.mybed1.txt.thin100.vcf.gz > sampleinfo.DP.txt
	${BCFTOOLS} query -f '%DP[\t%GQ]\n' ${PREFIX}.mybed1.txt.thin100.vcf.gz > sampleinfo.GQ.txt
	${BCFTOOLS} query -f '%POS\n' ${PREFIX}.mybed1.txt.thin100.vcf.gz > sampleinfo.pos.txt
	wait
...
if [[ "$calcdepth2" = TRUE ]]
	...
	echo "Thinning data using vcftools..."	
	${VCFTOOLS} -c --thin 100 --gzvcf masked_${PREFIX}.mybed1.txt.vcf.gz --recode --recode-INFO-all | gzip > ${PREFIX}.mybed1.txt.thin100.vcf.gz
	echo "Extracting depth information from thinned dataset using bcftools query..."
	${BCFTOOLS} query -f '%DP[\t%DP]\n' ${PREFIX}.mybed1.txt.thin100.vcf.gz > sampleinfo.DP.txt
	${BCFTOOLS} query -f '%DP[\t%GQ]\n' ${PREFIX}.mybed1.txt.thin100.vcf.gz > sampleinfo.GQ.txt
	${BCFTOOLS} query -f '%POS\n' ${PREFIX}.mybed1.txt.thin100.vcf.gz > sampleinfo.pos.txt
