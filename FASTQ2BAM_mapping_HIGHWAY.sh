...
REFERENCE=/ddn/data/fjsq43/Genomes/YarkandDeer/GCA_010411085.1_CEY_v1_genomic.fna
REFBN=/ddn/data/fjsq43/Genomes/YarkandDeer/GCA_010411085.1_CEY_v1_genomic
INPUTDIR=/ddn/data/fjsq43/Genomes/Wapiti/Rawreads
OUTPUTDIR=/ddn/data/fjsq43/Genomes/Wapiti/map2elaphus
...
SAMTOOLS=/ddn/data/fjsq43/Programs/variantcalling/samtools-1.3/samtools
PICARD=/opt/software/picard-2.26.5/picard.jar
BEDTOOLS=bedtools
QUALIMAP=/opt/software/qualimap/qualimap_v2.2.1/qualimap	# only needed for optional quality control step

NRCORES=4						# Number of cores or threads per file. For example: if working on 15 samples, and if specifying 4 cores, then usage of up to 60 cores.
...
qualitycontrol=FALSE			# step 8: quality control
removerepeats=FALSE				# step 9: remove reads which mapped to a repetitive region
indexbam3=FALSE					# step 10: index repeatmasked bam files
#######################
...
if [[ "$mapdata" = TRUE ]]
	...
	for bn in $(cat subsetaa)
		do
		echo $bn
		bwa mem $REFERENCE -1 ${INPUTDIR}/${bn}_1.fq.gz -2 ${INPUTDIR}/${bn}_2.fq.gz -t $NRCORES | ${SAMTOOLS} sort -o ${OUTPUTDIR}/${bn}.sorted.bam &
...
if [[ "$removedup" = TRUE ]]
	...
	for bn in $(cat allsamples.bn.txt)
		do
		echo $bn
		java -Xmx16g -jar ${PICARD} MarkDuplicates MAX_FILE_HANDLES_FOR_READ_ENDS_MAP=1000 I=${OUTPUTDIR}/${bn}.sorted.RG.bam O=${OUTPUTDIR}/${bn}.sorted.RG.dupremoved.bam M=${OUTPUTDIR}/${bn}.mark_dup_metrics.txt REMOVE_DUPLICATES=True &
...
if [[ "$qualitycontrol" = TRUE ]]
	...
	for bn in $(cat allsamples.bn.txt)
		...
		$QUALIMAP bamqc -bam ${OUTPUTDIR}/${bn}.sorted.RG.dupremoved.filtered.bam -outdir ${OUTPUTDIR}/bamqc_output -outfile ${bn}_qualimap2.pdf -nt 12 -nw 30000 --paint-chromosome-limits --java-mem-size=15G
		done
	wait
